...
namespace StringNS {
    class string {
      ...
      public:
        ...
        string(const char * lpsz){ text = (char*)lpsz; length = strlen(text); }
        string(char * lpsz){ text = lpsz; length = strlen(text); }
        string(char * lpsz, int len){ text = lpsz; length = len; }
        void FromChars(char * lpsz){ text = lpsz; length = strlen(text); }
...
namespace StringNS {
    ...
    string seek_first_word(string src, int * idx=null){
        int i = idx==null? 0 : *idx; int endl = i;
...
namespace StringNS {
    StringNS::string seek_first_csv_word(StringNS::string src, int * idx=null, bool trim_spaces=false){
        int i = idx==null? 0 : *idx; int endl = i;
...
namespace StringNS {
    StringNS::string seek_first_general_word(char sep[4], StringNS::string src, int * idx=null, bool trim_spaces=false){
        int i = idx==null? 0 : *idx; int endl = i;
        while (i < src.length && src.text[i] && (src.text[i]=='\r'||src.text[i]=='\n')) i++;
        int e = i; int in_quote = 0;
        while (e < src.length && src.text[e]){
